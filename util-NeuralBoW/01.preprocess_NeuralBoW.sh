...
for topk in `seq 1 100`; do
    echo $topk
    mkdir ${DIR}.$MODEL.oracle/srctrg_top$topk/
    mkdir ${DIR}.$MODEL.NBoW/srctrg_top$topk/
    for tvt in dev test train_h40000
    do
        python 01.preprocess_NeuralBoW.py   -prefix         ${CORPUS}_$tvt \
                                            -in_dir         ${DIR}.$MODEL/merge_srctrg.top100/ \
                                            -in_suffix_src  en.with_match.tkn \
                                            -in_suffix_trg  ja.with_match.tkn \
                                            -out_oracle_dir ${DIR}.$MODEL.oracle/srctrg_top$topk/ \
                                            -out_NBoW_dir   ${DIR}.$MODEL.NBoW/srctrg_top$topk/ \
